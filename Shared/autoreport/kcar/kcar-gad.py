...
class AutoReportUtils:
    ...
    """
    @staticmethod
    def parse_date(data_interval_end: str, minus_day: int, **kwargs) -> str:
        """
        ...
        try:
            ...
            # (!) additional subtraction e.g. washing
            if "minus_week" in kwargs:
                data_interval_end = data_interval_end - timedelta(weeks=kwargs["minus_week"])
...
three_weeks_ago = datetime.strftime(datetime.now() + timedelta(hours=9) - timedelta(days=1) - timedelta(weeks=3), "%Y-%m-%d")
...
try:
    ...
    three_weeks_ago = AutoReportUtils.parse_date(dbutils.widgets.get("data_interval_end"), 1, minus_week=3) # D-22
