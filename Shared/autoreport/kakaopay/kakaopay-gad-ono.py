...
class QueryGenerator(Enum):
    GET_CAMPAIGN = f"""
        ...
        WHERE 
          campaign.advertising_channel_type IN ('DISPLAY', 'DISCOVERY', 'VIDEO')
    """
...
#             spark.sql(f"delete from {self.table_path} where segmentDate = '{self.yesterday}'")
            spark.sql(f"delete from {self.table_path} where segmentDate = '{self.stats_date}'")
        except Exception as e:
            log.error(f"[FAIL-{sys._getframe().f_code.co_name.upper()}]{self.table_path}")
            raise e
...
# KAKAOPAY
google_ads_api = GoogleAdsApi("8483382344")

# try:
#     google_ads_api.proc_all()
# except Exception as e:
#     log.error("[FAIL-KAKAOPAY-GAD-ONO]")
#     raise e

try:
    google_ads_api.proc_all()
except Exception as e:
    if "Partition column `segmentDate` not found in schema []" in str(e):
        log.info("[FAIL-KAKAOPAY-GAD-ONO]NO-DATA")
    else:
        log.error("[FAIL-KAKAOPAY-GAD-ONO]")
        raise e
