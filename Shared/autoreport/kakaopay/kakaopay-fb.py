...
#     CAMPAIGNS = "ad.effective_status=['ACTIVE', 'PAUSED', 'PENDING_REVIEW', 'DISAPPROVED', 'PREAPPROVED', 'PENDING_BILLING_INFO', 'CAMPAIGN_PAUSED', 'ARCHIVED', 'ADSET_PAUSED', 'IN_PROCESS', 'WITH_ISSUES', 'DELETED']&limit=1000"
#     ADCREATIVES = f""
...
#     CAMPAIGNS_INSIGHTS = "time_range={since:'2022-10-13',until:'2022-10-16'}&level=ad&time_increment=1&limit=1000"
#     CAMPAIGNS_INSIGHTS = "date_preset=yesterday&breakdowns=impression_device&level=ad&time_increment=1&limit=1000"
#     CAMPAIGNS_INSIGHTS = "breakdowns=impression_device&level=ad&time_increment=1&limit=1000"
    ADS_INSIGHTS = "time_increment=1&level=ad&limit=1000"
...
#         self.today = datetime.strftime(datetime.now() + timedelta(hours=9), "%Y-%m-%d")
        self.yesterday = datetime.strftime(today - timedelta(days=1), "%Y-%m-%d")
        self.last_week_wash = datetime.strftime(today - timedelta(days=8), "%Y-%m-%d")
#         self.two_weeks_wash = datetime.strftime(datetime.now() + timedelta(hours=9) - timedelta(days=15), "%Y-%m-%d")
        
        self.base_path = "/dbfs/FileStore/configs/"
        self.config_name = "facebook_marketing_api_client_ptbwa1.pickle"
...
#             print(request_params)
            ...
            request_url = self.create_request_url(insights)
            response = requests.get(url=request_url, params=request_params)
            result = response.json()
#             print(result)
...
#             return getattr(ParamGenerator, domain).value
            if "INSIGHTS" in domain:
                return f"time_range={{since:'{self.stats_date}',until:'{self.stats_date}'}}&" + getattr(ParamGenerator, domain).value
#                 return f"time_range={{since:'{self.since_date}',until:'{self.until_date}'}}&" + getattr(ParamGenerator, domain).value
            else:
                return getattr(ParamGenerator, domain).value
        except Exception as e:
            log.error(f"[FAIL-{sys._getframe().f_code.co_name.upper()}]{domain}")
    ...
    def create_json_rdd(self):
        ...
        try:
            ...
            df_prefix = "wash_" if self.wash else ""
            setattr(self,  f"{df_prefix}df", spark.read.json(jsonRdd))
...
#         spark.catalog.dropGlobalTempView(raw_view)
#         spark.catalog.dropGlobalTempView(wash_view)
        
        raw_df = spark.sql(f"select * from auto_report.fb_kakaopay_ad_stats where dateStart='{self.stats_date}'")
        raw_df.createOrReplaceTempView(raw_view)
#         getattr(self, f"{self.domain.lower()}_df").createGlobalTempView(wash_view)
        self.df.createOrReplaceTempView(wash_view)
        ...
        query = f"""
            SELECT 
                ...
                w.reach,
                w.spend,
                w.videoP100WatchedActions,
                w.videoP25WatchedActions,
                w.videoP50WatchedActions,
                w.videoP75WatchedActions,
                w.videoPlayActions
            FROM
                (
                    SELECT 
                    ...
                    FROM 
                        {raw_view}
                    WHERE 
                        (adId, adsetId, campaignId, dateStart) IN
                        (
                            SELECT 
                                distinct adId,
                                        adsetId, 
            ...
            LEFT JOIN
                {wash_view} AS w
            ON 
                r.campaignId = w.campaignId
                AND r.adsetId = w.adsetId
                AND r.adId = w.adId    
                AND r.dateStart = w.dateStart    
            ...
            FROM 
                {raw_view}
            WHERE (adId, adsetId, campaignId, dateStart) NOT IN 
                (
...
#         setattr(self, f"{self.domain.lower()}_df", spark.sql(query))
        self.df = spark.sql(query)
    
    def add_columns(self):
        self.df = self.df.withColumn('washDate', lit(self.today))
...
#         return f"{self.base_db}.{self.channel}_{self.advertiser}_ad_stats"
        if self.wash :
            return f"{self.base_db}.{self.channel}_{self.advertiser}_{self.domain.lower()}_wash_stats"
        else :
            return f"{self.base_db}.{self.channel}_{self.advertiser}_{self.domain.lower()}_stats"
    ...
    def save_to_delta(self):
        ...
        try:
            ...
            (getattr(self, f"{df_prefix}df")
                 .drop("impressionDevice")
                 .write
                 .mode("append")
...
#             spark.sql(f"delete from {self.table_path} where dateStart = '{self.yesterday}'")
            spark.sql(f"delete from {self.table_path} where dateStart = '{self.stats_date}'")
#             spark.sql(f"delete from {self.table_path} where dateStart between '{self.since_date}' and '{self.until_date}'")
        except Exception as e:
            log.error(f"[FAIL-{sys._getframe().f_code.co_name.upper()}]{self.table_path}")
            raise e
...
# KAKAOPAY
customer_id = "589909145601881"
facebook_marketing_api = FacebookMarketingApi(customer_id)

# COMMAND ----------

facebook_marketing_api.yesterday
...
try:
    ...
    log.error("[FAIL-KAKAOPAY-FB]")
    raise e

# COMMAND ----------

# facebook_marketing_api.wash_df.display()
# spark.read.json(facebook_marketing_api.jsonRdd).display()

# COMMAND ----------

# spark.sql("delete from auto_report.fb_kakaopay_ad_wash_stats where dateStart between '2023-03-24' and '2023-03-26'")
...
# df = spark.sql("select * from auto_report.fb_kakaopay_ad_stats")