...
log = log4jLogger.LogManager.getLogger("KAKAOPAY")
...
date_bf8d = datetime.strftime((datetime.now(KST) - timedelta(8)), "%Y-%m-%d")
print('today',today)
print('date',date)
print('date_bf8d',date_bf8d)

# COMMAND ----------

try:
    df = spark.sql(f"DELETE FROM {database}.kakaopay_report_d WHERE date >= '{date}' OR Network_sub IS NULL")
    log.info(f"[SUCCESS-DELETE-SERIES-REPORT-D]{df.collect()}")
except Exception as e:
    print(e)
...
try:
    ...
    log.info(f"[SUCCESS-INSERT-SERIES-GAD-ACI-REPORT]{df.collect()}")
except Exception as e:
    print(e)
...
try:
    insert_query = f"""
        ...
        """            
    df = spark.sql(insert_query)
    log.info(f"[SUCCESS-INSERT-SERIES-GAD-ACE-REPORT]{df.collect()}")
except Exception as e:
    print(e)
    ...
    insert_query = f"""
            INSERT INTO {database}.series_report_d
                    ...
                    FROM (
                        ...
                        LEFT JOIN (
                            SELECT Date, Network_sub, OS_Name, CampaignName, AdgroupName, creativeName, SUM(installs) AS installs, SUM(reattributions) AS reattributions
                        ...
                        LEFT JOIN (
                            SELECT Date, Network_sub, OS_Name, CampaignName, AdgroupName, creativeName, 
                                ...
                                SUM(Use_Cookie_CU) AS Use_Cookie_CU, SUM(Use_Cookie) AS Use_Cookie, SUM(CV_CU) AS CV_CU, SUM(CV) AS CV
            ...
            UNION ALL
                    ...
                    FROM (
                        ...
                        LEFT JOIN
                            {database}.ad_series_map_media_network e
                        ON REPLACE(a.Network_sub, 'Facebook_SNS', 'Meta_Traffic') = e.`매체`
        ...
        """            
    df = spark.sql(insert_query)
    log.info(f"[SUCCESS-INSERT-SERIES-FB-REPORT]{df.collect()}")
except Exception as e:
    print(e)
...
try:
    insert_query = f"""
        ...
        """            
    df = spark.sql(insert_query)
    log.info(f"[SUCCESS-INSERT-SERIES-ASA-REPORT]{df.collect()}")
except Exception as e:
    print(e)
...
try:
    insert_query = f"""
            INSERT INTO {database}.series_report_d
                    ...
                    FROM (
                        SELECT
                            ...
                            SUM(NVL(metricsBilledChargeLocalMicro, 0))*1.12/1000000 AS spend
                        FROM {database}.twt_series_ad_stats
                    ...
                    LEFT JOIN (
                        SELECT Date, Network_sub, CampaignName, AdgroupName, creativeName, SUM(installs) AS installs, SUM(reattributions) AS reattributions
                        FROM {database}.ad_series_deliver_stats
                        WHERE Date >= '{date}'
            ...
            UNION ALL
                    ...
                    FROM (
                        SELECT
                            ...
                            SUM(NVL(metricsVideoViews75, 0)) AS videoP75,
                            SUM(NVL(metricsVideoViews100, 0)) AS videoP100,
                            SUM(NVL(metricsBilledChargeLocalMicro, 0))*1.12 AS spend
                        FROM {database}.twt_series_organic_stats
                        WHERE segmentDate >= '{date}'
                        GROUP BY
                                segmentDate,
                                Network_sub, OS_Name,
        ...
        """            
    df = spark.sql(insert_query)
    log.info(f"[SUCCESS-INSERT-SERIES-TWT-REPORT]{df.collect()}")
except Exception as e:
    print(e)
    ...
    insert_query = f"""
            INSERT INTO {database}.series_report_d
                    ...
                    FROM (
                        ...
                        LEFT JOIN (
                            SELECT Date, Network_sub, CampaignId, CampaignName, AdgroupId, AdgroupName, creativeId, creativeName, SUM(installs) AS installs, SUM(reattributions) AS reattributions
                            FROM {database}.ad_series_deliver_stats
                            WHERE Date >= '{date}'
        ...
        """            
    df = spark.sql(insert_query)
    log.info(f"[SUCCESS-INSERT-SERIES-TIKTOK-REPORT]{df.collect()}")
except Exception as e:
    print(e)
