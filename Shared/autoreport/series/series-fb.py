...
class AutoReportUtils:
    ...
    def parse_date(data_interval_end: str, minus_day: int) -> str:
        ...
        try:
            ...
            f_log.error(f"[FAIL-{sys._getframe().f_code.co_name.upper()}]")
            raise e
...
#     env = "dev"
    ...
    f_log.info("[FAIL-GET-ENV-PARAMS]THIS-IS-PROD-ACTION(DEFAULT)")
...
#     data_interval_end = AutoReportUtils.parse_date("2023-03-02 14:00:00", 1)
    ...
    f_log.info("[FAIL-GET-DATA-INTERVAL-END-PARAMS]THIS-IS-PROD-ACTION(DEFAULT)")
...
class ParamGenerator(Enum):
    ...
    CAMPAIGNS = "effective_status=['ACTIVE','PAUSED']&limit=60"
    ADCREATIVES = f""
    ADS_INSIGHTS = "time_range={since:'2022-08-30',until:'2022-08-30'}&breakdowns=impression_device&level=ad"
...
#     CAMPAIGNS_INSIGHTS = "time_range={since:'2022-08-30',until:'2022-08-30'}&breakdowns=impression_device&level=campaign&time_increment=1&limit=5000&filtering=[{field: 'ad.effective_status', operator:'IN', value:['ACTIVE', 'PAUSED', 'ARCHIVED', 'WITH_ISSUES', 'DELETED', 'CAMPAIGN_PAUSED']}]"
    CAMPAIGNS_INSIGHTS = "time_range={since:'2022-08-30',until:'2022-08-30'}&date_preset=yesterday&breakdowns=impression_device&level=ad&time_increment=1&limit=5000"
...
# MAGIC - App Type
# MAGIC   - Business
# MAGIC - V2 :: add time_range setting by data_interval_end
# MAGIC   - `2023-03-02`
...
#         WidgetValidator.is_valid_property(self.args)
    ...
    def get_campaigns(self):
        ...
        try:
            ...
            f_log.error(f"[FAIL-GET-CAMPAIGNS]")
...
#             params = self.get_params_str(domain)
    ...
    def get_insights(self, domain):
        ...
        try:
            ...
            # (!) add date_interval_end
            request_params["time_range"] = json.dumps({"since": self.yesterday, "until": self.yesterday})
...
#                 "access_token": "EAAGqdAK3ZAeYBAG6ZCAPaOQzYObGxfwRcfPP4NEGrEvKm17Or04XTZAc62ZCJfW6a9tXgKevgUaXhbqiX4VHZBMFORGctLteY2Q5pMYUWxQFUDYPB9C9gkqaZCex87DlZAFWCqkMYk2dedHP41kCe6k0TStXpcMjuL0uM7HZBE4ZBYzyCnkZA9VuR47XtVyUPBf6QSmshrD0bdpJ2UWCElWCiRC7EzwQMivboZD"
            }
...
try:
    ...
    if "Partition column `dateStart` not found in schema" in str(e):
        f_log.info("[FAIL-SERIES-FB]NO-DATA")
    else:
        f_log.error("[FAIL-SERIES-FB]")
        raise e
