...
try:
    df = spark.sql(f"DELETE from cream.propfit_ga_gongconects_daily WHERE date = '{criteria_date}'")
    k_log.info(f"[SUCCESS-DELETE-PROPFIT-GA-GONGCONECTS-DAILY]{df.collect()}")
except Exception as e:
    print(e)
    k_log.error(f"[FAIL-DELETE-PROPFIT-GA-GONGCONECTS-DAILY]{e}")
    raise e
...
try:
    insert_query = f"""
            INSERT INTO cream.propfit_ga_gongconects_daily
                    SELECT DISTINCT
                        jd.*,
                        gt.user_pseudo_id,
                        gt.ga_session_id,
                    ...
                    LEFT JOIN (
                        SELECT *
                        FROM cream.ga_gongconects_traffic
                        WHERE user_pseudo_id IN (SELECT DISTINCT user_pseudo_id FROM cream.ga_gongconects_traffic
    ...
    k_log.info(f"[SUCCESS-INSERT-PROPFIT-GA-GONGCONECTS-DAILY]{df.collect()}")
except Exception as e:
    print(e)
    k_log.error(f"[FAIL-INSERT-PROPFIT-GA-GONGCONECTS-DAILY]{e}")
    raise e
