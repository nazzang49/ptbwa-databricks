...
with open("/dbfs/FileStore/configs/bq_ga4_migun_config.json", "r") as f:
    config = json.load(f)
...
try:
    ...
    SELECT
        ...
        max(percent_scrolled) percent_scrolled                  -- scroll수정 2023-02-20
    FROM (
        SELECT
            ...
            if(b.key='percent_scrolled', b.value.int_value, null) as percent_scrolled                  -- scroll수정 2023-02-20
        FROM
...
# (spark
#     .read
#     .table("ice.migun_daily")
#     .withColumn("percent_scrolled", col("percent_scrolled").cast("long"))
#     .write
#     .format("delta")
#     .mode("overwrite")
#     .option("overwriteSchema", "true")
#     .saveAsTable("ice.migun_daily")
# )